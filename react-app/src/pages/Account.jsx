...
export default function Account() {
	...
	const createTicket = async (event) => {
		...
		setNewTicket({ subject: '', category: 'general', message: '' }); setShowTicketComposer(false); const notified = await notifyTicketReply(session, { ticketId: id }); await load(); setTab('tickets'); setStatus((s) => ({ ...s, error: notified ? '' : 'Ticket saved, but the email notification could not be sent.', message: `Ticket ${String(id).slice(0, 8)} created.` }));
	};
	const sendReply = async (event) => {
		...
		try { const message = await writeTicket(session, { action: 'message', ticketId: selectedTicket.id, message: reply.trim() }); setReply(''); const notified = await notifyTicketReply(session, { messageId: message.id }); await openTicket(selectedTicket); await load(); setStatus((s) => ({ ...s, error: notified ? '' : 'Reply saved, but the email notification could not be sent.' })); }
		catch (error) { setStatus((s) => ({ ...s, error: error.message })); }
		finally { replyInFlight.current = false; setReplySending(false); }
	};
