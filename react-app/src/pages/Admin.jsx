...
export default function Admin() {
	...
	const [tickets, setTickets] = useState([]); const [orders, setOrders] = useState([]); const [customers, setCustomers] = useState([]); const [access, setAccess] = useState(null); const [selected, setSelected] = useState(null); const [messages, setMessages] = useState([]); const [reply, setReply] = useState(''); const [error, setError] = useState(''); const [replySending, setReplySending] = useState(false);
	...
	const send = async (event) => { event.preventDefault(); if (replySending) return; setReplySending(true); try { await staffRequest(session, 'reply', { method: 'POST', body: { ticketId: selected.id, message: reply.trim() } }); setReply(''); await open(selected); await load(); } catch (sendError) { setError(sendError.message); } finally { setReplySending(false); } };
