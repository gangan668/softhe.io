...
export default function ResetPassword() {
	...
	return <div className="portal-page auth-page"><section className="portal-card auth-card"><h1>Choose a new password</h1><form className="portal-form" onSubmit={submit}><label>New password<input type="password" minLength="12" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} required /><small>{passwordRequirements}</small></label><button className="btn btn-primary" disabled={loading}>Update password</button>{status && <div role="status">{status}</div>}</form><Link to="/account">Return to account</Link></section></div>;
}
