...
describe('benchmark evidence validation', () => {
	it('calculates and verifies medians from two runs per profile', () => {
		const result = validateBenchmarkEvidence(validManifest);
		expect(result.errors).toEqual([]);
		expect(result.medians).toEqual(validManifest.reportedMedians);
	});

	it('rejects fewer than two runs', () => {
		const manifest = structuredClone(validManifest);
		manifest.stock.runs.pop();
		expect(validateBenchmarkEvidence(manifest).errors).toContain('stock.runs must contain at least two runs');
	});
